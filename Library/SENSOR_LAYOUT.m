...
% Date        : 01/01/2021
% Description : Functions for specifying the layout of sensors around the ego vehicle
% --------------------------------------------------------------------------------------------------------------------------------------------------------------
classdef SENSOR_LAYOUT
    methods(Static)
        ...
        % ======================================================================================================================================================
        function [FOVPtsEGOframe] = TransformFOVtoEGOframeCamera(CameraParam, nCameras, nCameraTypes, nPts)
            FOVPtsEGOframe = struct;
            FOVPtsEGOframe.Xcoord = single(0);
            FOVPtsEGOframe.Ycoord = single(0);
            FOVPtsEGOframe = FOVPtsEGOframe(ones(1, nCameras));
            CameraFOVPtsCartesian = SENSOR_LAYOUT.GenerateFOVBoundaryPoints(CameraParam.Intrinsic, nCameraTypes, nPts);
            for idx = 1:nCameras
                camType = CameraParam.Extrinsic(idx).SensorType;
                PtsLocalFrame  = [CameraFOVPtsCartesian(camType).XCoord(1,:); CameraFOVPtsCartesian(camType).YCoord(1,:)];
                RotMatrix = CameraParam.Extrinsic(idx).RotMat2D; TranslationVector = CameraParam.Extrinsic(idx).TranslationVec;
        ...
        % ======================================================================================================================================================
        function [FOVPtsEGOframe] = TransformFOVtoEGOframeRadar(RadarParam, nRadars, nRadarTypes, nPts)
            FOVPtsEGOframe = struct;
            FOVPtsEGOframe.Xcoord = single(0);
            FOVPtsEGOframe.Ycoord = single(0);
            FOVPtsEGOframe = FOVPtsEGOframe(ones(1, nRadars));
            RadarFOVPtsCartesian = SENSOR_LAYOUT.GenerateFOVBoundaryPoints(RadarParam.Intrinsic, nRadarTypes, nPts);
            for idx = 1:nRadars
                radType = RadarParam.Extrinsic(idx).SensorType;
                PtsLocalFrame  = [RadarFOVPtsCartesian(radType).XCoord(1,:); RadarFOVPtsCartesian(radType).YCoord(1,:)];
                RotMatrix = RadarParam.Extrinsic(idx).RotMat2D; TranslationVector = RadarParam.Extrinsic(idx).TranslationVec;
                PtsEGOframe = RotMatrix * PtsLocalFrame + TranslationVector;
                FOVPtsEGOframe(idx).Xcoord = PtsEGOframe(1,:);
                FOVPtsEGOframe(idx).Ycoord = PtsEGOframe(2,:);
            end
